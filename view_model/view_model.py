...
from consts import page


async def show_low_data_item(client: TelegramClient, chat, response, buttons):
    for item in response:
        try:
            await client.send_message(chat.id, item.to_string(), file=item.get_url(), link_preview=False, buttons=buttons)
        except:
...
async def show_search_data_item(client: TelegramClient, chat, response, buttons):
    ...
    for item in response:
        try:
            search_items.append([Button.text(f"{item.rawTitle} | {item.type} | {item.rating.imdb} | {item.year}", resize=True)])
        except:
            print("An exception occurred")
