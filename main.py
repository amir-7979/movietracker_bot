...
from view_model.view_model import *
...
def get_new_data():
    ...
    print(time.ctime())
    threading.Timer(wait_seconds, get_new_data).start()
...
async def get_news_with_data(t_client, channel):
    ...
    while True:
        ...
        await show_low_data_item(t_client, channel, response, click_keyboard_button(False, State.main))
        if len(response) != 12:
            return
        i = i + 1
...
@client.on(events.NewMessage(pattern=""))
async def rec_commands(event):
    global function_number, is_searching
    message = event.message
    text = message.text
    chat = await event.get_chat()
    if text == '🔥 News':
        ...
        response = await func_list(function_number)
